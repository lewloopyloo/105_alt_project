	...
	m_overlayTitle(m_font)
{
	...
	// Use window size early so we can centre main menu buttons
	auto ws = m_window.getSize();
	sf::Vector2f windowSize{ static_cast<float>(ws.x), static_cast<float>(ws.y) };

	// --- Setup the single Level Select button (main menu) ---
	const float mainBtnWidth = 216.f;
	const float mainBtnHeight = 80.f;
	const float mainBtnSpacing = 16.f; // spacing between the two main buttons

	// centre the pair of buttons vertically and horizontally
	...
	// Label: fit and center inside Level Select button
	m_levelSelectLabel.setString("Level Select");
	m_levelSelectLabel.setCharacterSize(20); // slightly reduced so it fits better
	...
	// Exit button sits directly under Level Select (closer together)
	...
	// Exit label: smaller and centred inside exit button
	...
	m_overlayFrame.setFillColor(sf::Color(30, 30, 30, 230)); // dark border/frame

	// panel (lighter background)
	...
	m_overlayPanel.setFillColor(sf::Color(240, 240, 240, 230)); // light panel

	// overlay title
	m_overlayTitle.setCharacterSize(28);
	m_overlayTitle.setString("Select Level");
	// center title at top of panel
	...
	// Place level buttons inside the panel
	// Starting Y offset below the title
	...
	m_playButton.setSize({ btnWidth, btnHeight });            // Level 1 inside overlay
	...
	m_play2Button.setSize({ btnWidth, btnHeight });           // Level 2 inside overlay
	...
	m_play3Button.setSize({ btnWidth, btnHeight });           // Level 3 inside overlay
...
void Menu::handleInput(float dt)
{
	...
	// If the overlay is visible, clicks select level choices (or clicking outside closes)
	if (m_showLevelMenu)
	{
		// Level 1
		...
		// Click outside panel closes the overlay
		if (m_input.isLeftMousePressed() &&
...
void Menu::render()
{
	...
	if (m_showLevelMenu)
	{
		// draw dim, frame, panel and the level choices
...
void Menu::update(float dt)
{
	...
	if (m_showLevelMenu)
	{
		// hover states for overlay level buttons
